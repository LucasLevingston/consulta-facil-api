...
class GoogleOAuthCallbackServiceTest {
    ...
    void execute_existingGoogleUser_returnsToken() {
        when(googleOAuthPort.exchangeCode("auth-code", googleProps.getRedirectUri())).thenReturn(info);
        when(userRepository.findByGoogleId("sub-123")).thenReturn(Optional.of(existingUser));

        var result = service.execute("auth-code");
